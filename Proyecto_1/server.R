server <- function(input, output, session) {
  ...
  summary_dataset <- reactive({ execute_safely(
    f_ds_rsm(raw_df, input$std_filter, input$time_slider_1, input$excluir_id_tienda_input)
  )
  })
  ...
  trend_dataset <- reactive({ execute_safely(
    f_ds_tnd(raw_df,input$user_id_input, input$shop_id_input, input$payment_input, input$time_slider_2)
  )
  })
  ...
  statistics_dataset <- reactive({ execute_safely(
    f_ds_stad(raw_df,input$user_id_input2, input$shop_id_input2, input$excluir_id_tienda_input2, "Todos", input$time_slider_3)
  )
  })
